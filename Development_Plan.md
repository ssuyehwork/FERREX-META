...
- **目标**：针对当前系统中非专业的逻辑架构设计，进行全方位的架构梳理，规划清晰分层，降低模块耦合，提升系统可维护性、可扩展性及高并发性能。
- **核心维度要求**：
    1. **清晰分层**：界定数据控制层（ScanController / MftReader 等）与视图渲染层（ScanTableModel / Viewports）的物理边界，严禁在 UI 线程与渲染组件中直接或高频交叉调用底层高开销的锁/磁盘IO逻辑。
    2. **低耦合高内聚**：解耦异步检索过滤与前台数据装饰属性，移除跨层共享的复杂中间状态。
    3. **可维护性与可扩展性**：建立标准的数据代理和视图接口，使后续新增视图模式（如滑块、卡片、自适应缩略图）或替换存储引擎（如 SQL / 磁盘USN）均能零侵入式插拔。
    4. **性能控制与零死锁**：重构多线程调度与取消原子，确保高频变焦、滚轮缩放与程序析构时绝不发生 UI 线程死锁或卡假死。
