...
- **核心解耦与优化要求**：
    - **模块职责彻底净化**：设计将缩略图异步提取、L1/L2 双轨缓存、LIFO 队列和硬件感知彻底移出适配器 `ScanTableModel`，打包高聚类集成至全局 `ThumbnailManager` 组件。
    - **消除对冲式读写大锁**：通过后台线程 1:1 投影离线数据快照（SoA 只读拼装）来断开 UI 主线程渲染与 MftReader 底层数据容器在运行时的锁竞争，消除微秒级潜在卡顿与假死，砸碎防抖和 QTimer 的狗皮膏药现状。
