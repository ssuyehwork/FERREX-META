...
- **核心要求**：
    ...
    - **去重置、全平滑渐进拉伸**：在滚轮和滑块尺寸高频变动的中断响应中，移除对 `clearThumbCache(true)` 的重复调用，限制 or 彻底消除此时对历史二级缓存的清空与重建抖动。在滚动变焦期间完全复用上一次的历史 Pixmap 资产由 Delegate 渲染层进行平滑缩放拉伸以填满卡片，绝不产生闪烁或空白卡片，且零开销、零竞争，确保缩放变焦过程如行温流水般丝滑。
...
### 12. 全局逻辑架构拼凑硬伤深度审计与解耦规划 (Plan-212)
- **目标**：彻底暴露整款应用在不同开发阶段由于多次迭代堆积和缺乏统一联审导致的多处上下文逻辑冲突、设计盲区与职责渗透。
- **核心解耦与优化要求**：
    - **模块职责彻底净化**：设计将缩略图异步提取、L1/L2 双轨缓存、LIFO 队列 and 硬件感知彻底移出适配器 `ScanTableModel`，打包高聚类集成至全局 `ThumbnailManager` 组件。
    - **消除对冲式读写大锁**：通过后台线程 1:1 投影离线数据快照（SoA 只读拼装）来断开 UI 主线程渲染与 MftReader 底层数据容器在运行时的锁竞争，消除微秒级潜在卡顿与假死，砸碎防抖 and QTimer 的狗皮膏药现状。
    - **对齐 dataChanged 交互契约**：对齐适配器的数据发射与各 ResultView 卡片更新对 roles 的精细化解析过滤规则，防止两边在不同阶段各自为战、静默吞更新或者无端引起全量卡片重新计算，大幅提升工业级系统的稳定性与维护性。

### 13. 卡片左上方“扩展名角标”彻底物理根除 (Plan-213)
- **目标**：解决在“自适应”和“网格”视图模式下，卡片左上方反反复复死灰复燃、恢复显示后缀名角标（如展示 DIR、PSD、PNG 等文字的圆角绿色色块）的犟种顽疾。
- **物理剔除要求**：
    - **行级物理销毁**：在 `ThumbnailDelegate.cpp` 的 `paint` 渲染热路径中，行级物理性剔除全部涉及绘制 `ext` 后缀及 `extRect` 矩形色块的代码（原第 131 至 148 行），不留控制尸体，不留 `//` 注释。
    - **红线核验阻断**：严格遵循 AGENTS.md 第 3.5 节，在未来任何架构合并中，一票否决并禁止任何后续 AI 脑补、恢复任何一个像素的后缀角标绘制，对越界脑补和复活废弃逻辑实施“反人类”级别红线阻断。

### 14. “自动显示”勾选卡死高精日志追踪与埋点方案 (Plan-214)
- **目标**：对“自动显示”复选框勾选后程序假死、Windows 提示无响应的问题进行微秒级高敏定位。
- **高敏日志埋点要求**：
    - **锁竞争耗时监控**：在 `MftReader::activeCount()` 内部加入 `QReadLocker` 读锁申请的微秒级监控和大于 5ms 严重卡顿报警。
    - **COM 提取超时监控**：在 `UiHelper::getShellThumbnail` 及 `ThumbnailWarmupPipeline` 预热流中，对 `IShellItemImageFactory::GetImage` 加入耗时阈值判定，一旦提取延迟大于 100ms 立刻发出耗时严重警告，暴露 COM 死锁和多线程卡顿源。
    - **搜索自激探测**：在 `ScanController::performSearch` 启动时输出带参数的全景日志，物理捕捉高频、自激触发的全量搜索流，找出无响应的症结。
