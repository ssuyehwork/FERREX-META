...
- **核心病因**：
    ...
    3. 哈希构建成本：ResultSet 中 200万项 of `unordered_map` 构建耗时过长。
...
- **核心要求**：
    ...
    - **去重置、全平滑渐进拉伸**：在滚轮和滑块尺寸高频变动的中断响应中，移除对 `clearThumbCache(true)` 的重复调用，限制或彻底消除此时对历史二级缓存的清空与重建抖动。在滚动变焦期间完全复用上一次的历史 Pixmap 资产由 Delegate 渲染层进行平滑缩放拉伸以填满卡片，绝不产生闪烁或空白卡片，且零开销、零竞争，确保缩放变焦过程如行温流水般丝滑。
...
### 12. 线程竞争、死锁、卡顿、高CPU和析构性能异常全面审计 (Plan-211)
- **目标**：完成对当前版本全模块的线程并发及资源调度缺陷进行深度排查和科学优先级管理。
- **排查及审计核心点**：
    1. **MftReader O(总量) compact 降级**：分析 `MftReader::compact` 在调用时对全量项重建耗时问题。
    2. **ThumbnailWarmupPipeline 重复扫描缺陷**：分析多线程预热对前 5000 条未作切分导致锁竞争和预热失败的病因。
    3. **ScanTableModel 与后台锁竞争**：分析主线程 `data()` 的 `getFullPath` 在高频滚动中由于 `m_dataLock` 被后台 USN 独占导致的瞬时挂起机制。
    4. **析构同步 waitForDone 同步死等**：分析退出程序时 Shell COM 阻塞导致的主线程秒级滞后。
    5. **UsnWatcher 自我写入过滤核实**：核对 USN 过滤器拦截黑名单的物理安全边界。
    6. **并发搜索与重排序取消轮询**：核对 `MemoryQueryEngine::search` 的取消检测频次以及 `ScanController::sort` 的原子取消遗失缺陷。
