...
## 1. 任务背景
在当前系统的视图切换逻辑中，存在着通过离散尺寸选项切换和通过连续滑杆切换这两套逻辑重合的设计。为了精简界面交互，让排版控制更为纯粹，将彻底移除菜单中多余的图标尺寸，并将视图功能重新划分为三种模式，即自适应、网格和列表。

## 2. 问题定位
* **关键源文件**：`src/ui/ScanDialog.cpp`
* **问题成因**：
  在 `ScanDialog.cpp` 中共有两处重复实现了“超大图标”、“大图标”、“中图标”等切换逻辑：
  1. **标题栏视图切换按钮弹出菜单**（位于 `ScanDialog::ScanDialog` 构造函数内约第 715 行至 775 行附近）：
     该处通过 `QList<ViewDef>` 循环渲染了“超大图标”、“大图标”和“中图标”三个选项，并另外在下方添加了“自适应”与“网格”单选。
  2. **主视图右键级联“视图(V)”菜单**（位于 `ScanDialog::onCustomContextMenu` 槽函数内约第 1730 行至 1775 行附近）：
     该处添加了“超大图标(X)”、“大图标(L)”、“中图标(M)”以及“详情(D)”/“列表”等动作，并在其后追加了“两端对齐 (不等宽)”与“网格排版 (等高宽)”选项。
  这两处逻辑导致大小配置（`iconSize`）与布局形式（`layoutMode`）强耦合，不仅带来了界面菜单的极度冗余，也与系统顶部的尺寸调节滑杆（“标记为①的滑杆”）存在功能上的冲突。

## 3. 强制对照表

| 编号 | 用户原话 / 我的理解 | 方案对应点 | 是否一致 |
|------|---------------------|------------|----------|
| 1    | 由于标记为①的滑杆是用来调整大小的，所以标记为②的这三个选项可以彻底移除了，我觉着它是冗余的 | 彻底移除标题栏下拉菜单以及主右键菜单级联“视图(V)”中的“超大图标”、“大图标”、“中图标”三个选项 | ✅ 一致 |
| 2    | 那我们就把它划分为三种模式，自适应、网格、列表 | 在上述两处菜单中，重新划分为自适应、网格、列表三种模式，并作为平级单选切换 | ✅ 一致 |

## 4. 详细解决方案

我们将在 `src/ui/ScanDialog.cpp` 的两处菜单构建逻辑中，剔除所有“超大/大/中图标”的硬编码尺寸选项，并直接以“自适应”、“网格”、“列表”三种一级平级状态来进行重写与整合。

### 4.1 标题栏视图按钮弹出菜单重构
在 `ScanDialog::ScanDialog` 中，重构 `viewBtn` 关联的弹出菜单构建。将原先嵌套大小的循环剔除，统一声明三个模式单选动作。
...
### 4.2 主视图右键上下文菜单重构
在 `ScanDialog::onCustomContextMenu` 中，同样彻底剥离冗余的图标尺寸项，只保留视图子菜单 “自适应(A)”、“网格(G)” 与 “列表(L)” 三种选项作为一级的单选。
...
**本次方案涉及范围：**
- [ ] `src/ui/ScanDialog.cpp`：构造函数视图切换按钮弹出菜单以及右键“视图(V)”级联子菜单的代码重构。
