...
QVariant ScanTableModel::data(const QModelIndex& index, int role) const {
    ...
    // 零锁物理快照核心突破：主线程 data() 绝对免锁且不再需要向 MftReader 发送任何检索请求，调用开销直接降为 0！
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        QString ext = m_currentResultSet->cachedExts[row];
        
        static const QSet<QString> thumbExts = {"psd", "ai", "eps", "jpg", "jpeg", "png", "webp", "svg"};
        if (thumbExts.contains(ext) && !m_currentResultSet->isDirFlags[row]) {
            ...
            QPixmap pix = ThumbnailManager::instance().requestThumbnail(key, m_currentResultSet->cachedPaths[row], ext, thumbSize, size, mtime);
            if (!pix.isNull()) {
                return pix;
            }
            return QVariant(); // 加载期提供空阻断，防止默认图标闪烁
        }
        
        // 常规不支持缩略图的后缀，直接放行，回退到系统默认图标
        return MftReader::instance().getCachedIcon(ext, m_currentResultSet->isDirFlags[row]);
        ...
        if (index.column() == 0 || m_currentResultSet->isDirFlags[row]) return QColor("#3498db");
        ...
        QString ext = m_currentResultSet->cachedExts[row].toLower();
...
void ScanTableModel::updateResults(std::shared_ptr<ResultSet> nextSet) {
    ...
    newSet->cachedNames = baseSet->cachedNames;
    newSet->cachedPaths = baseSet->cachedPaths;
    newSet->cachedSizes = baseSet->cachedSizes;
    newSet->cachedMtimes = baseSet->cachedMtimes;
    newSet->isDirFlags = baseSet->isDirFlags;
    newSet->cachedExts = baseSet->cachedExts;
