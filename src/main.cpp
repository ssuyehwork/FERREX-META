...
void customMessageHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
    Q_UNUSED(context); 
    QFile logFile("FERREX-META_debug.log");
...
int main(int argc, char *argv[]) {
    ...
    a.setApplicationName("FERREX-META");
    a.setOrganizationName("FERREX-METATeam");

    // 2026-05-27 物理修复：在主线程预热元数据管理器单例
    // 确保其内部的 QTimer 等对象归属于主线程，避免跨线程创建导致的行为不确定性
    FERREX-META::MetadataManager::instance();

    // 2. 初始化数据库 (仅核心表结构，必须同步完成)
    std::wstring dbPath = L"FERREX-META.db";
    if (!FERREX-META::Database::instance().init(dbPath)) {
    ...
    FERREX-META::ScanDialog* w = new FERREX-META::ScanDialog();
    w->show();

    // 4. 集成托盘图标支持
    FERREX-META::TrayController* tray = new FERREX-META::TrayController(w);
    tray->show();

    // 5. 启动异步系统扫描（后台初始化，UI 可响应）
    FERREX-META::CoreController::instance().startSystem();
